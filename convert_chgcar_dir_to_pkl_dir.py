...
if __name__ == "__main__":
    ...
    if workers <= 1:
        for f in tqdm(chgcar_files):
            _convert(f)
        count_elements_in_numpy_files(file_list_path=filelist_file,file_list_path_bak=filelist_file, workers=1)
    else:
        with Pool(workers) as p:
            p.map(_convert, chgcar_files)
        count_elements_in_numpy_files(file_list_path=filelist_file, file_list_path_bak=filelist_file, workers=workers)
