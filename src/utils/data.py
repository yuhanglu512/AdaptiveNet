...
def read_MAGMOM(filepath):
    with open(filepath, "r") as f:
        task=json.load(f)
    try:
        spin=np.array([i['tot'] for i in task['calcs_reversed'][0]['output']['outcar']['magnetization']])
    except KeyError:
        spin=0
    return spin
...
def read_Q(filepath):
    with open(filepath, "r") as f:
        task=json.load(f)
    try:
        q=np.array([i['tot'] for i in task['calcs_reversed'][0]['output']['outcar']['charge']])
    except KeyError:
        q=0
    return q
...
def load_magmom(root, mpid):
    root = Path(root)
    mpid = Path(mpid)
    magmom_file = str(root / f"{mpid}_MAGMOM.pkl")
    with open(magmom_file, "rb") as f:
        magmom = pickle.load(f)
    return magmom

def load_Q(root, mpid):
    root = Path(root)
    mpid = Path(mpid)
    magmom_file = str(root / f"{mpid}_Q.pkl")
    with open(magmom_file, "rb") as f:
        magmom = pickle.load(f)
    return magmom
