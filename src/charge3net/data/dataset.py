...
from src.charge3net.data.graph_construction import GraphConstructor, AtomGraphConstructor, Atomic_Q_Constructor
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if q and not density:
            ...
            if "inference" in str(self.data_root).lower():
                dataset = DensityData(self.data_root)
                subsets = split_data(dataset, val_frac=self.val_frac, split_file=self.split_file)
                self.train_set = AtomicDataset(subsets["train"], self.gc, spin=True, filename='train')
                self.val_set = AtomicDataset(subsets["validation"], self.gc, spin=True, filename='validation')
                self.test_set = AtomicDataset(subsets["test"], self.gc, spin=True, filename='test')
            else:
                dataset=DensityData(self.data_root,read_spin=True, q = True)
                subsets = split_data(dataset, val_frac=self.val_frac, split_file=self.split_file)
                self.train_set = Atomic_Q_Dataset(subsets["train"], self.gc, spin=True, filename='train')
                self.val_set = Atomic_Q_Dataset(subsets["validation"], self.gc, spin=True, filename='validation')
                self.test_set = Atomic_Q_Dataset(subsets["test"], self.gc, spin=True, filename='test')
        elif mag:
            self.gc=graph_constructor()
            dataset=DensityData(self.data_root,read_spin=True)
            subsets = split_data(dataset, val_frac=self.val_frac, split_file=self.split_file)
            self.train_set = AtomGraphDataset_mag(subsets["train"], self.gc, spin=True, filename='train')
            self.val_set = AtomGraphDataset_mag(subsets["validation"], self.gc, spin=True, filename='validation')
            self.test_set = AtomGraphDataset_mag(subsets["test"], self.gc, spin=True, filename='test')
        else:
            self.train_gc = graph_constructor(num_probes=train_probes, spin=self.spin)
            self.val_gc = graph_constructor(num_probes=val_probes, spin=self.spin)
            self.test_gc = graph_constructor(num_probes=test_probes, spin=self.spin)
            q = False if "inference" in str(self.data_root).lower() else q
            dataset = DensityData(self.data_root,self.spin, q)
            subsets = split_data(dataset, val_frac=self.val_frac, split_file=self.split_file)
            self.train_set = DensityGraphDataset(subsets["train"], self.train_gc, spin=self.spin)
            self.val_set = DensityGraphDataset(subsets["validation"], self.val_gc, spin=self.spin)
...
class Atomic_Q_Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, graph_constructor, grid_size_file=None, max_grid_size=1e7, spin=False, filename='train', **kwargs):
        ...
        data_file = f"models/q_data_{filename}.npz"
        ...
        if not os.path.isfile(data_file):
            ...
            for index in self.dataset_indices:
                ...
                graph_dict_all.append(graph_dict)
            np.savez(data_file, graph_dict_all=graph_dict_all)
            # 直接将数据存储在内存中
            self.graph_dict = graph_dict_all
        else:
            # 使用上下文管理器加载数据，并立即将数据提取到内存中
            with np.load(data_file, allow_pickle=True) as data:
                # 提取数据到内存中，而不是保持对npz文件的引用
                self.graph_dict = data['graph_dict_all'].tolist()
    def __len__(self):
...
class AtomGraphDataset_mag(torch.utils.data.Dataset):
    # This wrapper dataset provides a workaround such that the number
    # of probes does not need to be specified in the collate_fn of the dataloader. 
    ...
    def __init__(self, dataset, graph_constructor, grid_size_file=None, max_grid_size=1e7, spin=False, filename='train', **kwargs):
        ...
        data_file = f"models/mag_data_{filename}.npz"

        if not os.path.isfile("models/mag_data_%s.npz"%(filename)):
            graph_dict_all=[]
            for index in self.dataset_indices:
                start_time = time.time()
...
class DensityGraphDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset, graph_constructor, grid_size_file=None, max_grid_size=1e7, spin=False, **kwargs):
        super().__init__(**kwargs)
        self.dataset = dataset
        self.graph_constructor = graph_constructor
        self.grid_size_file = grid_size_file
    ...
    def __getitem__(self, index):
        ...
        if len(data_dict["density"].shape) == 4 and self.spin:
            grid_shape = torch.tensor(data_dict["density"].shape)
        elif len(data_dict["density"].shape) == 4 and not self.spin:
            grid_shape = torch.tensor(data_dict["density"].shape[:-1])
        else:
            grid_shape = torch.tensor(data_dict["density"].shape)
        if self.splitting_mode:
            # reshape density and grid position to allow for arbitrary splitting
            if len(data_dict["density"].shape) == 4 and self.spin: # spin density TODO: have actual arg for spin
                density = data_dict["density"].reshape(-1,1,1,2)[probe_offset:probe_offset+self.max_grid_size]
            elif len(data_dict["density"].shape) == 4 and not self.spin:
                density = data_dict["density"][...,0].reshape(-1,1,1)[probe_offset:probe_offset+self.max_grid_size]
            else:
        ...
        if 'q' in data_dict.keys():
            graph_dict.update(q=data_dict['q'])
...
class DensityData(torch.utils.data.Dataset):
    def __init__(self, datapath, read_spin=False, q=False, **kwargs):
        super().__init__(**kwargs)
        if os.path.isfile(datapath) and datapath.endswith(".tar"):
            self.data = DensityDataTar(datapath,read_spin)
        elif os.path.isfile(datapath) and datapath.endswith("filelist.txt"):
            self.data = DensityPickleDir(datapath, spin=read_spin, q= q)
            ...
            self.data = ConcatDataset([DensityData(path,read_spin) for path in filelist]) 
...
class DensityPickleDir(torch.utils.data.Dataset):
    ...
    '''
    def __init__(self, filename, spin=False, q=False, **kwargs):
        super().__init__(**kwargs)
    ...
    def __getitem__(self, index):
        ...
        if self.spin:
            magmom = utils.load_magmom(root=self.root, mpid=member)
        if self.q:
            q = utils.load_Q(root=self.root, mpid=member)

        if self.spin and self.q:
            return {
                "density": density,
                "atoms": atoms,
                "origin": origin,
...
class DensityDataTar(torch.utils.data.Dataset):
    def __init__(self, tarpath, spin=False,**kwargs):
        super().__init__(**kwargs)
    ...
    def extract_member(self, tarinfo):
        with tarfile.open(self.tarpath, "r") as tar:
            ...
            if tarinfo.name.endswith((".cube", ".cube.gz", "cube.zz", "cube.lz4")):
                density, atoms, origin = utils.read_cube(filecontent,read_spin=self.spin)
            else:
                density, atoms, origin = utils.read_vasp(filecontent,read_spin=self.spin)
