...
class AtomGraphConstructor(object):
    ...
    def __call__(self,
        ...
        graph_dict = self.atoms_and_probes_to_graph(atoms)
        ...
        graph_dict.update(
            magmom=torch.tensor(MAGMOM, dtype=self.default_type),
        )
            
        return graph_dict
    
    
class Atomic_Q_Constructor(object):
    def __init__(self, cutoff, num_probes=None, disable_pbc=False, sorted_edges=False):
        super().__init__()
        self.cutoff = cutoff
        self.disable_pbc = disable_pbc
    ...
    def __call__(self,
        ...
        if MAGMOM is not None and q is not None:
            graph_dict.update(
                magmom=torch.tensor(MAGMOM, dtype=self.default_type),
                q=torch.tensor(q, dtype=self.default_type),
            )
            
        return graph_dict
    

    def atoms_and_probes_to_graph(self, atoms):
...
class GraphConstructor(object):
    def __init__(self, cutoff, num_probes=None, disable_pbc=False, sorted_edges=False, spin=False):
        super().__init__()
        self.cutoff = cutoff
        self.disable_pbc = disable_pbc
        self.sorted_edges = sorted_edges
    ...
    def __call__(self,
        ...
        if self.spin:
            graph_dict.update(
                node_vector=torch.tensor(MAGMOM, dtype=self.default_type),
            )
    ...
    def sample_probes(self, grid_pos, density):
        ...
        #    if np.max(density[...,1])< abs(np.min(density[...,1])):
        #        probe_target[...,1]= -probe_target[...,1]
