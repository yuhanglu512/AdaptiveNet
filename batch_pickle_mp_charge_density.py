...
from src.utils.data import decompress_file, read_vasp, load_numpy_density, read_MAGMOM, read_Q
from tqdm import tqdm
import numpy as np
import pickle

def main(mp_chg_dir, mp_pkl_dir, my_task_id, num_tasks, read_spin):
    mp_pkl_dir=Path(mp_pkl_dir)
    mp_chg_dir=Path(mp_chg_dir)
    ...
    for fname in tqdm(my_fnames, desc="Job files"):
        ...
        # load data, dump data to pickle with np.save
        density_file = (mp_pkl_dir / mpid).with_suffix(".npy")
        atoms_file = (mp_pkl_dir / f'{mpid}_atoms').with_suffix(".pkl")
        MAGMOM_file = (mp_pkl_dir / f'{mpid}_MAGMOM').with_suffix(".pkl")
        q_file = (mp_pkl_dir / f'{mpid}_Q').with_suffix(".pkl")
        if not (density_file.exists() and density_file.stat().st_size > 10 * 1024 and atoms_file.exists()):
            dec = decompress_file(str(chgcar_file))
            try:
                density, atoms, origin = read_vasp(dec, read_spin=read_spin)
            except ValueError as e:
                print(f"Error reading {fname.stem}: {e}")
                with open(err_file, "a+") as f:
                    f.write(f"{fname.stem} Value error\n")
                continue
        ...
        if not MAGMOM_file.exists():
            magmom=read_MAGMOM(fname/"task_doc.json")
            with open(MAGMOM_file, "wb") as f:
                pickle.dump(magmom, f)
        
        if not q_file.exists():
            q = read_Q(fname/'task_doc.json')
