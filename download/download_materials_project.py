...
def write_chgcar(chgcar: Chgcar, root_dir: Union[Path, str] , mpid: str):
    ...
    chgcar_path = write_dir / "CHGCAR"
    if chgcar_path.exists() and chgcar_path.stat().st_size > 1 * 1024 * 1024:
        print(f"File {chgcar_path} already exists and is larger than 1MB, skipping write.")
        return
...
def write_task_id_to_txt(taskdoc: TaskDoc, root_dir: Union[Path, str], mpid: str):
    ...
    if fname.exists():
        print(f"File {fname} already exists, skipping write.")
        return
    with open(fname, "w") as f:
        f.write(f"{taskdoc.task_id}\n")
        
def write_task_doc(taskdoc: TaskDoc, root_dir: Union[Path, str], mpid: str):
    r'''Write task_doc to json file'''
    fname = Path(root_dir) / mpid / "task_doc.json"
    if fname.exists():
        print(f"File {fname} already exists, skipping write.")
        return
    with open(fname, "w") as f:
        json.dump(taskdoc.dict(), f, indent=4)
...
def _read_in_write_out(mp_api_key: str, mpid: str, outpath: Union[Path, str]):
    ...
    CHGCAR_path = outpath / mpid / "CHGCAR"
    taskid_path = outpath / mpid / "task_id.txt"
    taskdoc_path = outpath / mpid / "task_doc.json"
    if CHGCAR_path.exists() and CHGCAR_path.stat().st_size > 100 * 1024 and taskid_path.exists() and taskdoc_path.exists():
        #print(f"Skipping {mpid}, files already exist.")
        return
    try:
    ...
    if taskdoc is not None:
        # This will record the taskID and write task docs
        write_task_id_to_txt(taskdoc=taskdoc, root_dir=outpath, mpid=mpid)
        write_task_doc(taskdoc=taskdoc, root_dir=outpath, mpid=mpid)
...
def _read_in_write_out_task(
    ...
    taskid_path = outpath / mpid / "task_id.txt"
    taskdoc_path = outpath / mpid / "task_doc.json"
    if CHGCAR_path.exists() and CHGCAR_path.stat().st_size > 1 * 1024 * 1024 and taskid_path.exists() and taskdoc_path.exists():
        print(f"Skipping {mpid}, files already exist.")
        return
